...
async function registryPanel() {
  ...
  return `<div class="ev-card ev-card-body" id="ev-research-registry"><div class="ev-kicker">Research process / inspectable history</div><h2>Research registry</h2><p>Protocols record source eligibility, grouped holdouts, baseline, fixed candidates, evaluation budget and promotion rules. Audit reservations cannot be reopened by changing a plan ID or data normalization.</p>
    ...
    <h3>What an improvement claim must pass</h3><p>Before a new audit can start, the registry checks the actual source, dataset, evaluator and model files and requires complete validation and audit predictions for the frozen samples. These checks establish file integrity and row coverage. They do not establish that training, source labels or candidate selection were scientifically valid.</p>
    <div class="ev-table-wrap"><table class="ev-table" id="ev-registry-controls"><thead><tr><th>Check</th><th>Current capability</th></tr></thead><tbody><tr><td>Archived file bytes match committed fingerprints</td><td>${registry.capabilities.artifactByteVerification?'Enforced for new audit starts':'Not connected'}</td></tr><tr><td>Predictions cover every required sample and method</td><td>${registry.capabilities.predictionMembershipChecks?'Enforced for new audit starts':'Not connected'}</td></tr><tr><td>Source labels, lineage and scientific eligibility</td><td>Source-specific evaluator required</td></tr><tr><td>Candidate selection and independent scoring</td><td>Not connected</td></tr><tr><td>Automatic promotion / learned research policy</td><td>Not connected</td></tr></tbody></table></div>
    <p class="ev-small" id="ev-registry-artifact-counts">${fmt(registry.artifactVerification?.boundProtocols??0)} protocols have used the new artifact binding; ${fmt(registry.artifactVerification?.predictionBundles??0)} audit prediction bundles are committed. The historical soil result remains under its original checks. No new scientific result is claimed by this software update.</p>
    ...
    <div class="ev-links"><a href="/data/evidence/${escape(manifest.registry.file)}" download>Download registry snapshot</a></div>${jsonDetails('Event history and integrity limits',{head:registry.head,events:registry.events,capabilities:registry.capabilities,artifactVerification:registry.artifactVerification,limitations:registry.limitations})}</div>`;
}
